...
The methodological framework of this study is illustrated in Figure 3 and comprises five stages: (1) data preprocessing and spectral standardization; (2) baseline chain construction, i.e., progressive comparison of Raw K-means, PCA + K-means, Canonical AE + K-means, and SAE + K-means; (3) Geo-DEC spatial enhancement extension, adding geological indices, neighborhood statistics, and DEC joint clustering objectives on top of the AE representation learning paradigm; (4) multi-metric evaluation and geological interpretation, including traditional cluster separability metrics, spatial continuity metrics, spectral response analysis, and automated mapping; (5) external mineral-map auxiliary validation, in which 1:250,000 hyperspectral mineral-distribution maps are georeferenced to the Sentinel-2 20 m grid to check spatial consistency between cluster patches and mineral-anomaly clues.
...
#table3(
  ...
  [Validation approach], [Clustering metrics + geological samples/knowledge], [Clustering metrics + false-color image comparison + hyperspectral mineral-map auxiliary validation],
...
#table3(
  ...
  [Geological interpretation mainly qualitative], [Quantitative analysis of 5 band ratios + fully automated mapping + external mineral-map overlay], [Corrected B11/B12 interpretation, retained C1 as a candidate clue requiring validation, and checked spatial consistency using mineral-anomaly maps],
)
]
#set text(size: 12pt)
...
#table3(
  ...
  [C1], [18.0%], [B11/B12=0.992, B12/B8A=1.282; B11/B12 below 1 does not prove B12 absorption], [Banded-blocky in the north, partly coincident with false-color brownish-red areas and locally comparable with the mineral map], [Alteration or lithological-boundary candidate clue], [Low to moderate; supported only by auxiliary mineral-map comparison and still requiring geological-map or field validation],
...
(7) External hyperspectral mineral maps can serve as auxiliary validation material for unsupervised cluster interpretation. After the 1:250,000 Geocloud hyperspectral mineral-distribution maps are georeferenced to the Sentinel-2 20 m grid, they can be used to inspect the spatial relationship between cluster patches and mineral-anomaly clues such as muscovite, chlorite, carbonate, and iron oxides. The current AOI is mainly covered by sheets J46 C 002003 and J46 C 002004, and the mosaicked effective coverage reaches 95.36%, with a small no-coverage gap preserved along the sheet boundary. Because these maps are not conventional lithological maps and the current data source is JPG format, this study uses them only as visual auxiliary validation rather than as strict accuracy-assessment ground truth.

Synthesizing the above findings, this paper distills four core insights: First, the SAE+K-means unsupervised clustering framework remains effective in hyper-arid porphyry copper districts and possesses cross-climate-zone and cross-tectonic-unit transferability. Second, in this data scenario---where Sentinel-2 10-band data are dominated by linear colinearity and the number of surface classes is limited (4--6)---the clustering performance of a shallow autoencoder is on par with that of a deep SAE, suggesting that the gain from autoencoder depth is scene-dependent. Third, band-ratio analysis and external hyperspectral mineral maps can provide useful candidate clues for geological interpretation, but they cannot be treated as high-confidence alteration or lithological indicators without correct ratio interpretation, spatial-distribution analysis, awareness of map-product limitations, and validation from geological maps or field observations. Fourth, Geo-DEC demonstrates that beyond the original two-stage framework, incorporating geological priors, spatial context, and clustering-objective joint optimization is an effective extension direction for improving the spatial continuity and interpretability of unsupervised geological mapping, though it does not replace AE/SAE as strong baselines on traditional cluster separability metrics. These insights respectively address the gaps in the original paper (Naagar et al., 2024) regarding cross-regional validation, depth-gain analysis, quantitative spectral interpretation, and spatial context utilization.
