...
#let en-workflow-fig-width = 8.2cm
#let en-paired-fig-width = 7.6cm
#let en-paired-curve-width = 7.4cm
#let en-ablation-fig-width = 7.3cm
...
Unsupervised clustering methods enable lithological mapping in remote, sparsely vegetated areas where field labels are scarce. However, existing deep-learning-based clustering frameworks have been validated primarily in semi-arid sedimentary terrains, and their applicability to hyper-arid porphyry copper districts---as well as the relative benefit of deep versus shallow autoencoders---remains unclear. This study uses a two-stage design for the Tuwu-Yandong porphyry copper district in Hami, Xinjiang, China. Using a Sentinel-2A L2A image (2024-08-13, Tile T46TFM) with 10 spectral bands (B02--B12) resampled to 20 m, we construct a dataset of 2,772,025 valid pixels after cloud, shadow, and water masking. First, we adapt the SAE+K-means framework of Naagar et al. (2024) and compare a baseline chain of Raw K-means, PCA + K-means, Canonical AE + K-means, and SAE + K-means to test whether nonlinear dimensionality reduction, network depth, and bottleneck dimension matter. Results show that PCA yields only marginal improvement over Raw K-means (CHI +2.7%), and the shallow Canonical AE matches or slightly exceeds the deep SAE in cluster separability (CHI 147,239 vs. 144,423), suggesting that shallow nonlinear transformations suffice for the main clustering structure of Sentinel-2 10-band data. SAE retains advantages in reconstruction fidelity (MSE 0.00285 vs. 0.00466) and cluster compactness (DBI 0.6981 vs. 0.7338). The ablation experiment reveals that $z=3$ causes severe class collapse (largest class 48.9%), while $z=5$ is used as the main compromise configuration. Second, we introduce Geo-DEC as a spatial-context enhancement beyond the pixel-independent AE+K-means pipeline. Geo-DEC achieves the highest local agreement (0.9348) and the lowest isolated pixel ratio (0.0057), although its CHI and DBI do not outperform AE/SAE, indicating that its main benefit is improved spatial continuity rather than stronger global cluster separation. Quantitative spectral analysis finds an anomalous ratio combination in C1 (B11/B12=0.992, B12/B8A=1.282); however, $"B11/B12" < 1$ means B11 is lower than B12 and does not prove B12 absorption, so this class is retained only as a candidate clue requiring geological-map or field validation. As an independent auxiliary reference, 1:250,000 hyperspectral mineral-distribution maps from China Geology Cloud (Geocloud) were georeferenced to the Sentinel-2 20 m grid and used for qualitative overlay validation. The study area is mainly covered by sheets J46 C 002003 and J46 C 002004, whereas the two southern adjacent sheets fall outside the AOI and contribute no valid pixels after clipping. The aligned mineral-map mosaic reaches 95.36% effective coverage within the AOI; the residual sheet-boundary gap is retained rather than artificially filled. This auxiliary mineral-anomaly map allows visual assessment of the spatial relationship between clusters and muscovite, chlorite, carbonate, and iron-oxide anomalies, but it is not used as lithological reference labels or as a basis for accuracy assessment. A fully scripted Python mapping pipeline replaces traditional GIS workflows for reproducible geological interpretation. This study validates the cross-regional applicability of the SAE+K-means framework in a hyper-arid porphyry copper setting and extends it with Geo-DEC for spatially coherent unsupervised lithological mapping.
]

#keywords(name: "Keywords")[Sentinel-2; lithological mapping; stacked autoencoder; canonical autoencoder; Geo-DEC; PCA; K-means clustering; spatial continuity; hyperspectral mineral-distribution map; porphyry copper deposit; Tuwu-Yandong]

= Introduction

Remote sensing has become an indispensable tool in regional geological surveys and mineral exploration due to its large-area coverage, multi-temporal capability, and low cost #super[1]. Traditional remote sensing geological interpretation largely relies on expert visual interpretation or supervised classification. However, in remote, sparsely vegetated regions where field labels are scarce, unsupervised clustering offers a unique advantage---it can automatically discover grouping structures in spectral features without prior labels, providing an objective initial classification reference for geological mapping #super[2].
...
(6) *External auxiliary validation*: Georeference and mosaic China Geology Cloud (Geocloud) 1:250,000 hyperspectral mineral-distribution maps to the Sentinel-2 20 m grid, and use them only for qualitative spatial-consistency checking between clusters and mineral-anomaly clues;

(7) *Full-pipeline reproducibility*: Construct a complete Python preprocessing and mapping pipeline from Sentinel-2 SAFE extraction to final geological interpretation maps, ensuring full experimental reproducibility.
...
As an external auxiliary reference for interpreting the unsupervised clustering results, this study also collected 1:250,000 Altun hyperspectral mineral-distribution maps released by China Geology Cloud (Geocloud). According to the AOI extent (approximately 94.30°E--94.70°E, 42.02°N--42.32°N) and the standard 1:250,000 sheet boundaries, the study area is mainly covered by the two northern sheets J46 C 002003 (93.0°E--94.5°E, 42.0°N--43.0°N) and J46 C 002004 (94.5°E--96.0°E, 42.0°N--43.0°N). The simultaneously downloaded southern adjacent sheets J46 C 003003 and J46 C 003004 lie at 41.0°N--42.0°N, south of the AOI, and therefore contribute no valid pixels after clipping. These maps express the spatial distribution of mineral assemblages such as muscovite, chlorite, carbonate, and iron oxides derived from hyperspectral mineral identification; they are remote-sensing mineral-anomaly maps rather than conventional lithological geological maps. Therefore, they are used only to assist visual checks of spatial correspondence between clusters and mineral-anomaly clues, not as reference labels for supervised accuracy assessment.

#grid(
  columns: (1fr, 1fr),
  gutter: 0.9cm,
  align: center + top,
  enfigure(image("../data/processed/preview_true_color_B04_B03_B02.png", width: en-paired-fig-width), [True-color composite of the study area (B04/B03/B02)]),
  enfigure(image("../data/processed/preview_geology_B12_B8A_B02.png", width: en-paired-fig-width), [Geological false-color composite of the study area (B12/B8A/B02).\
...
The implementation workflow of this study is illustrated in Figure 3 and comprises five modules: (1) data preprocessing and spectral standardization; (2) baseline chain construction, i.e., progressive comparison of Raw K-means, PCA + K-means, Canonical AE + K-means, and SAE + K-means; (3) Geo-DEC spatial enhancement extension, adding geological indices, neighborhood statistics, and DEC joint clustering objectives on top of the AE representation learning paradigm; (4) multi-metric evaluation and geological interpretation, including traditional cluster separability metrics, spatial continuity metrics, spectral response analysis, and automated mapping; (5) external mineral-map auxiliary validation, in which 1:250,000 hyperspectral mineral-distribution maps are georeferenced to the Sentinel-2 20 m grid to check spatial consistency between cluster patches and mineral-anomaly clues. These modules implement the two-stage experimental logic described above while also documenting the auxiliary validation and reporting workflow.
...
The methodological framework of this study references the open-source implementation of Naagar et al. (2024) (https://github.com/sydney-machine-learning/autoencoders_remotesensing). The original framework combines PCA, canonical autoencoders, and stacked autoencoders with K-means to generate clustered maps and interpret them as lithological maps. This study first reproduces and transfers this pipeline to the Tuwu-Yandong study area, then adds the Geo-DEC spatial enhancement experiment on top of its pixel-independent processing. Table 3 shows the correspondence between our implementation and the original framework, where PCA / Canonical AE / SAE correspond to the original pipeline and Geo-DEC is our extension.
...
This section organizes the experimental results according to the two-stage logic. §4.2--§4.4 first report the baseline results of Raw K-means, PCA + K-means, SAE, and Canonical AE to address questions of spectral representation learning, network depth, and bottleneck dimension; §4.5--§4.6 introduce Geo-DEC in the same metrics table and result figures, but with interpretation emphasis shifted toward spatial continuity and map readability; §4.7--§4.8 conduct spectral response analysis and geological interpretation mapping based on the main clustering results; §4.9 provides qualitative auxiliary validation using hyperspectral mineral-distribution maps.
...
#grid(
  ...
  gutter: 0.9cm,
  align: center + top,
  enfigure(image("../results/cluster_sae/sae_training_loss_z4.png", width: en-paired-curve-width), [SAE $z=4$ training loss curve (30 epochs)]),
  enfigure(image("../results/cluster_sae/sae_training_loss_z5.png", width: en-paired-curve-width), [SAE $z=5$ training loss curve (30 epochs).\
...
#grid(
  ...
  gutter: 0.8cm,
  align: center + top,
  enfigure(image("../results/cluster_sae/sae_kmeans_k5_z3.png", width: en-ablation-fig-width), [Ablation experiment: SAE $z=3$, $k=5$ clustering result.\
  The green class expands massively (48.9%), with clear class collapse.]),
  enfigure(image("../results/cluster_sae/sae_kmeans_k6_z4.png", width: en-ablation-fig-width), [Ablation experiment: SAE $z=4$, $k=6$ clustering result.\
...
#block(width: 100%)[
  #set align(center)
  #enfigure(image("../results/cluster_sae/sae_kmeans_k6_z5.png", width: en-ablation-fig-width), [Ablation experiment: SAE $z=5$, $k=6$ clustering result.\
...
== External Auxiliary Validation Using Hyperspectral Mineral-Distribution Maps

In the absence of field samples and directly digitizable large-scale lithological maps, this study further introduces 1:250,000 hyperspectral mineral-distribution maps from China Geology Cloud (Geocloud) as an independent auxiliary data source for qualitative spatial-consistency validation. Four adjacent sheets, J46 C 002003, J46 C 002004, J46 C 003003, and J46 C 003004, were downloaded, and their spatial relationship to the Tuwu-Yandong AOI was first checked from the standard sheet boundaries. For each JPG-format sheet, the main map frame was cropped to remove white page margins and part of the layout frame. The cropped maps were then georeferenced according to their sheet longitude-latitude ranges, reprojected to the Sentinel-2 UTM Zone 46N grid (EPSG:32646) at 20 m resolution, and clipped to the study area. The result shows that the current AOI is mainly covered by the two northern sheets J46 C 002003 and J46 C 002004, while J46 C 003003 and J46 C 003004 are south of the AOI and contribute no valid pixels after AOI clipping. The georeferenced mineral-anomaly map reaches 95.36% effective coverage within the AOI. The remaining uncovered area is mainly located along the boundary between the two valid sheets, and this gap is preserved rather than artificially filled. Finally, the SAE $z=5$, $k=6$ clustering result is overlaid semi-transparently on the mineral-anomaly map to produce Figure 17.

#enfigurewide(image("../results/geology_validation/mineral_maps/mineral_map_validation_panel.png", width: en-wide-fig-width), [Qualitative spatial-consistency validation between the Geocloud hyperspectral mineral-distribution map and the SAE clustering result.\
(a) Mineral-anomaly map after cropping the main map frames, georeferencing, mosaicking, and clipping to the Tuwu-Yandong AOI. The AOI is mainly covered by sheets J46 C 002003 and J46 C 002004, with a small retained no-coverage gap along the sheet boundary. (b) SAE $z=5$, $k=6$ clustering result overlaid on the mineral-anomaly map.])

It should be clarified that Figure 17 is intended to assist interpretation of the spatial relationship between unsupervised cluster patches and mineral-anomaly clues such as muscovite, chlorite, carbonate, and iron oxides. It does not treat the mineral-anomaly map as a conventional lithological map or as reference labels for supervised classification. The two datasets represent fundamentally different objects. The SAE $z=5$, $k=6$ clustering result is derived from 10 broad Sentinel-2 bands and reflects integrated spectral-spatial surface units controlled by visible--NIR--SWIR reflectance, SWIR band differences, and spatial context. In contrast, the hyperspectral mineral-distribution maps are based on narrower diagnostic absorption features and express spatial distributions of mineral assemblages. Because the two differ in spectral resolution, classification target, and analytical granularity, a one-to-one correspondence between cluster labels and mineral classes should not be expected. This mismatch is a methodological expectation rather than evidence of clustering error.

Under this interpretation framework, if a cluster is spatially adjacent to or locally coincident with hydroxyl-mineral or iron-oxide anomaly belts in the hyperspectral mineral-distribution map, the cluster may be considered to have independent auxiliary evidence supporting a possible alteration-related or lithological meaning. Conversely, if a class mainly falls in broad background areas or no-data regions of the mineral-anomaly map, it should not be overinterpreted geologically. Because the current source maps are JPG layout products, graticules, sheet-boundary lines, and minor layout-edge remnants may remain after registration; the small uncovered gap near 94.5°E is retained in the result to avoid mistaking interpolation or nearest-neighbor filling for real mineral information. Therefore, this section positions Figure 17 as qualitative spatial-consistency validation rather than quantitative accuracy assessment.

In summary, the purpose of this auxiliary validation is not to evaluate classification accuracy, but to examine whether the surface-unit differentiation revealed by unsupervised SAE clustering shows reasonable spatial consistency with existing hyperspectral mineral-anomaly patterns. If original Geocloud mineral-class rasters or vectorized mineral layers become available in the future, a stricter quantitative cross-analysis could be conducted by building cluster-mineral contingency tables and computing the area proportion of dominant mineral classes within each cluster.
...
#table3(
  ...
  [Validation approach], [Clustering metrics + geological samples/knowledge], [Clustering metrics + false-color image comparison + hyperspectral mineral-distribution map auxiliary validation],
...
#table3(
  ...
  [Geological interpretation mainly qualitative], [Quantitative analysis of 5 band ratios + fully automated mapping + external mineral-anomaly-map overlay], [Corrected B11/B12 interpretation, retained C1 as a candidate clue requiring validation, and checked spatial consistency using mineral-anomaly maps],
)
]
#set text(size: 12pt)
...
#table3(
  ...
  [C1], [18.0%], [B11/B12=0.992, B12/B8A=1.282; B11/B12 below 1 does not prove B12 absorption], [Banded-blocky in the north, partly coincident with false-color brownish-red areas and locally comparable with the mineral-anomaly map], [Alteration or lithological-boundary candidate clue], [Low to moderate; supported only by auxiliary mineral-anomaly-map comparison and still requiring geological-map or field validation],
...
(1) *Cluster 1 should be treated as a candidate clue rather than a confirmed alteration zone.* The corrected physical interpretation is important: B11/B12=0.992 is the lowest among the six classes, but $"B11/B12" < 1$ means B11 is lower than B12 and therefore cannot prove B12 absorption. The support for an alteration-related interpretation comes mainly from three aspects: first, this class is spatially associated with part of the brownish-red areas in the geological false-color image (Figure 2), although that tone mainly reflects B12-channel brightness/contrast and is not equivalent to SWIR absorption; second, wall-rock alteration in the Tuwu-Yandong porphyry copper deposit is dominated by sericitization and propylitization and commonly occurs as belts or patches near contacts between porphyry bodies and wall rocks #super[5], broadly resembling the northern banded-blocky distribution of C1; third, the external hyperspectral mineral-distribution map (Figure 17) can provide auxiliary reference for local hydroxyl-mineral or iron-oxide anomalies, but because it is a JPG-format remote sensing mineral-anomaly product rather than a conventional lithological map, this evidence can only be used for spatial-consistency checking. Overall, C1 may be retained as an alteration or lithological-boundary candidate clue, but its confidence should not be described as high and must be further constrained by published geological maps or field verification.
...
It must be emphasized that the above geological interpretation is spectral inference based on unsupervised clustering results. The hyperspectral mineral-distribution map provides external mineral-anomaly clues, but it remains a remote-sensing interpretation product and cannot replace lithological geological maps or field samples. Therefore, definitive geological attribution still requires field verification or calibration against published large-scale geological maps.

== Limitations and Future Work

This study has the following limitations: (1) Unsupervised clustering lacks field validation; the geological meaning of cluster classes requires subsequent calibration. (2) The newly introduced hyperspectral mineral-distribution map auxiliary validation is still based on JPG layout products. Although the main map frames were cropped and part of the page margins and frame lines were removed, graticules, sheet-boundary lines, and small no-coverage areas may remain after registration. Moreover, these maps express remote-sensing mineral anomalies rather than conventional lithological units, so they can only support visual spatial-consistency checks and cannot serve as strict accuracy-assessment reference data. (3) Geo-DEC has introduced 3×3 statistical spatial context but is still not end-to-end convolutional spatial modeling; future work can further incorporate convolutional autoencoders (CAE) or graph neural networks to jointly utilize larger-scale spectral-spatial information. (4) The current version of Geo-DEC has not undergone systematic ablation of DEC loss weight, 3×3 spatial window, and enhanced feature combinations; therefore, this study can only state that "spatial continuity improves while traditional separability metrics decline under the current configuration," and cannot precisely decompose the individual contributions of geological indices, spatial statistics, and DEC loss. (5) Both K-means and DEC favor compact cluster structures and have limited adaptability to elongated or arc-shaped cluster distributions that may exist in spectral space; GMM or spectral clustering may be more suitable alternatives. (6) Only single-season summer imagery was used; seasonal differences in solar elevation angle and surface moisture content may affect spectral characteristics. (7) Only bottleneck settings with $z ≤ 5$ were tested; exploration of larger dimensions is insufficient.

Future work can proceed in the following directions: (1) Introduce DEM and topographic factors (slope, aspect, shadow correction) to eliminate topographic effects. (2) Use ASTER thermal infrared bands to supplement lithologically sensitive emissivity information. (3) Obtain original Geocloud mineral-class rasters, vectorized mineral layers, or large-scale conventional geological maps, and conduct quantitative consistency assessment between clustering results and external maps using contingency tables, Adjusted Rand Index (ARI), or Normalized Mutual Information (NMI). (4) Conduct component ablation around Geo-DEC, separately testing "geological indices only," "spatial context only," "AE + K-means on 28D features," "different DEC weights," and "different neighborhood window sizes." (5) Compare more dimensionality reduction methods (t-SNE, UMAP, Variational Autoencoder VAE) for their effect on clustering. (6) Validate the generalization capability of this framework across a broader extent of the East Tianshan region.

= Conclusions

Taking the Tuwu-Yandong porphyry copper district in Xinjiang as the study area, this paper constructs a complete remote sensing geological analysis pipeline from preprocessing to unsupervised lithological clustering based on 10 spectral bands (B02--B12) of Sentinel-2A L2A imagery. The study adopts a two-stage logic: first transferring and comparing a spectral clustering baseline chain comprising Raw K-means, PCA + K-means, Canonical AE + K-means, and SAE + K-means; then introducing Geo-DEC as a spatial-context enhancement extension to test its effectiveness in improving geological map-unit continuity. The main conclusions are as follows:
...
(7) External hyperspectral mineral-distribution maps can serve as auxiliary validation material for unsupervised cluster interpretation. After the 1:250,000 Geocloud maps are georeferenced to the Sentinel-2 20 m grid, they can be used to inspect the spatial relationship between cluster patches and mineral-anomaly clues such as muscovite, chlorite, carbonate, and iron oxides. The current AOI is mainly covered by sheets J46 C 002003 and J46 C 002004, and the mosaicked effective coverage reaches 95.36%, with a small no-coverage gap preserved along the sheet boundary. Because these maps are not conventional lithological maps and the current data source is JPG format, this study uses them only as visual auxiliary validation rather than as strict accuracy-assessment reference data.

Synthesizing the above findings, this paper distills four core insights: First, the SAE+K-means unsupervised clustering framework remains effective in hyper-arid porphyry copper districts and possesses cross-climate-zone and cross-tectonic-unit transferability. Second, in this data scenario---where Sentinel-2 10-band data are dominated by linear colinearity and the number of surface classes is limited (4--6)---the clustering performance of a shallow autoencoder is on par with that of a deep SAE, suggesting that the gain from autoencoder depth is scene-dependent. Third, band-ratio analysis and external hyperspectral mineral-distribution maps can provide useful candidate clues for geological interpretation, but they cannot be treated as high-confidence alteration or lithological indicators without correct ratio interpretation, spatial-distribution analysis, awareness of map-product limitations, and validation from geological maps or field observations. Fourth, Geo-DEC demonstrates that beyond the original two-stage framework, incorporating geological priors, spatial context, and clustering-objective joint optimization is an effective extension direction for improving the spatial continuity and interpretability of unsupervised geological mapping, though it does not replace AE/SAE as strong baselines on traditional cluster separability metrics. These insights respectively address the gaps in the original paper (Naagar et al., 2024) regarding cross-regional validation, depth-gain analysis, quantitative spectral interpretation, and spatial context utilization.
