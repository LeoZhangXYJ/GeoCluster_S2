...
同时，Geo-DEC 将 K-means 的后处理式聚类推进为 DEC 式联合优化：先用 K-means 初始化隐空间聚类中心，再通过软分配和目标分布的 KL 散度微调编码器与聚类中心。这一设计试图缓解传统 AE + K-means 中“先重建、后聚类”的目标错位问题。然而，本文实验结果显示，Geo-DEC 并未在 CHI 和 DBI 等传统分离指标上超过 Canonical AE 或 SAE——其 CHI（68,821）甚至低于 Raw K-means 基线（123,898），降幅达 44.5%。这一大幅退化值得深入分析。可能的原因包括：① 28 维输入特征中，3×3 邻域均值和标准差等空间上下文特征本质上是一种局部平滑，虽然改善了空间连续性，但可能稀释了原始光谱中具有判别力的细微差异（如 SWIR 吸收特征），导致隐空间中不同类别的光谱判别信息被平滑抹平；② DEC 损失权重（0.1）和 3×3 邻域窗口等超参数尚未经过系统消融，当前配置可能未达到最优平衡。这本质上揭示了一个空间连续性与光谱分离性之间的 trade-off：引入空间上下文能够降低椒盐噪声，但过度平滑会牺牲光谱判别力。

此外，Geo-DEC 采用了更深的网络结构（28→64→32→z→32→64→28，8,231 个参数）和更大的输入维度，但其聚类分离性不升反降，这与 Stage 1 的核心发现——Sentinel-2 数据场景下增加网络深度对聚类分离性增益不明显——高度吻合。换言之，无论是在 10 维原始光谱上叠加 SAE 的深度增益（Stage 1），还是在 28 维增强特征上叠加 Geo-DEC 的深层联合优化（Stage 2），额外的网络深度均未带来聚类分离性方面的实质性提升。这一跨阶段的一致性进一步支持了本文的论点：该数据场景下的聚类判别结构主要被浅层非线性变换所捕获，更深层的网络在重建保真度和空间连续性等“外围”指标上仍有改善空间，但不应被指望成为提升聚类分离性的主要手段。
...
（2）非线性自编码器表示有效。Canonical AE 和 SAE 的主要配置均显著优于 Raw K-means 和 PCA，表明自编码器的重建目标能够保留对聚类判别有用的低方差信息，如 SWIR 吸收相关的波段差异。
...
（4）Bottleneck 维度显著影响聚类质量。$z=3$ 导致严重类别塌缩（最大类占比 48.9%，CHI 骤降至 93,656），$z=5$ 在类别均衡性、聚类指标和光谱细节保留之间取得更好平衡，适合作为本研究区的主实验隐空间维度。

（5）光谱比值分析为地质解译提供了定量支撑。Cluster 1（18.0%）的 B11/B12=0.992 明确了 B12（2190 nm）处的 OH⁻ 吸收特征，结合其与假彩色图中褐红色蚀变信号的空间一致性，可高置信度解译为绢英岩化/青磐岩化蚀变带候选区。
...
综合以上发现，本文凝练出四条核心认识：第一，SAE+K-means 无监督聚类框架在极端干旱斑岩铜矿区依然有效，具备跨气候带和跨大地构造单元的迁移能力；第二，在该数据场景下——Sentinel-2 10 波段以线性共线性为主导、地表类别数有限（4--6 类）——浅层自编码器的聚类性能与深层 SAE 旗鼓相当，提示自编码器深度的增益具有场景依赖性；第三，B11/B12 波段比值可作为该区域热液蚀变的快速遥感指示标志；第四，Geo-DEC 表明在原有两阶段框架之外，引入地质先验、空间上下文和聚类目标联合优化是提升无监督地质制图空间连续性和可解释性的有效扩展方向，但并不替代 AE/SAE 作为传统聚类分离指标上的强基线。上述认识分别对应了原论文（Naagar et al., 2024）在跨区域验证、深度增益分析、定量光谱解译和空间上下文利用方面的不足。
