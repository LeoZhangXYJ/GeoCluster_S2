...
#figurex(image("../data/processed/preview_true_color_B04_B03_B02.png"), [研究区真彩色合成影像（B04/B03/B02）])

#figurex(image("../data/processed/preview_geology_B12_B8A_B02.png"), [研究区地质假彩色合成影像（B12/B8A/B02）。\
...
#figurex(image("assets/image.png"), [整体技术流程图])
...
#figurex(image("../results/cluster_baseline/elbow_curve.png"), [Baseline K-means Elbow 曲线（$k=2"–"12$）。\
...
#figurex(image("../results/cluster_pca/pca_explained_variance.png"), [PCA 方差解释分析。\
...
#figurex(image("../results/cluster_sae/sae_training_loss_z4.png"), [SAE $z=4$ 训练损失曲线（30 epochs）])

#figurex(image("../results/cluster_sae/sae_training_loss_z5.png"), [SAE $z=5$ 训练损失曲线（30 epochs）。\
...
#figurex(image("../results/cluster_ae/ae_training_loss_z5.png"), [Canonical AE $z=5$ 训练损失曲线（30 epochs）。\
...
#v(0.5em)
#tablecaption[表 9 全部实验配置的聚类指标对比]
...
#figurex(image("../results/cluster_sae/sae_kmeans_k5_z3.png"), [消融实验：SAE $z=3$, $k=5$ 聚类结果。\
绿色类大面积扩张（占比 48.9%），类别塌缩明显。])

#figurex(image("../results/cluster_sae/sae_kmeans_k6_z4.png"), [消融实验：SAE $z=4$, $k=6$ 聚类结果。\
最大类占 42.0%，较 $z=3$ 改善但仍偏高，出现两个 ~4.7% 小类。])

#figurex(image("../results/cluster_sae/sae_kmeans_k6_z5.png"), [消融实验：SAE $z=5$, $k=6$ 聚类结果。\
最大类占 37.2%，类别均衡性最优，空间连续性良好。])

=== 四种方法综合对比

#figurex(image("../results/final_figures/method_comparison_2x2.png"), [四种方法聚类结果对比。\
...
#figurex(image("../results/final_figures/final_comparison_2x3.png"), [综合对比图（2×3）。\
...
#figurex(image("../results/cluster_spectra/cluster_spectral_profiles.png"), [各聚类类别在 10 个波段上的平均光谱反射率曲线（SAE $z=5$, $k=6$）。\
阴影带表示 ±1 标准差。灰色区域标注了 SWIR 波段（B11: 1610 nm, B12: 2190 nm），该区域对含 OH⁻/CO₃²⁻ 的蚀变矿物具有特征吸收。\
Cluster 1 在 B11--B12 区间呈现最显著的斜率下降，指示 SWIR 吸收特征。])

#v(0.5em)
#tablecaption[表 10 各聚类类别的关键波段比值（均值）]
...
#figurex(image("../results/cluster_spectra/cluster_spectral_profiles_per_class.png"), [各聚类类别的单独光谱响应曲线（含 ±1σ 误差带）])
...
#figurex(image("../results/final_figures/python_final_interpretation_map.png"), [基于 Python 自动生成的遥感地质解释图。\
...
#v(0.5em)
#tablecaption[表 12 聚类类别综合地质解译（基于光谱响应 + 空间分布 + 区域地质知识）]
