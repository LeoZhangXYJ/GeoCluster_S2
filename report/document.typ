...
为补充无监督聚类结果的外部地质约束，本文还收集了地质云发布的 1:25 万阿尔金高光谱遥感矿物分布图。根据研究区范围（约 94.30°E--94.70°E, 42.02°N--42.32°N）与 1:25 万标准图幅范围，AOI 实际跨越 J46 C 002003（93.0°E--94.5°E, 42.0°N--43.0°N）和 J46 C 002004（94.5°E--96.0°E, 42.0°N--43.0°N）两幅北侧图件；同时下载的 J46 C 003003 与 J46 C 003004 位于 41.0°N--42.0°N，低于本研究区南界，因此在当前 AOI 裁剪结果中不贡献有效像元。该类图件基于高光谱遥感矿物识别结果表达白云母、绿泥石、碳酸盐、铁氧化物等矿物组合的空间分布，性质上属于遥感矿物异常图，而非传统岩性地质图。因此，本文仅将其用于辅助检查聚类结果与矿物异常线索之间的空间对应关系，不将其作为监督分类精度评价的真值标签。
...
本文的方法框架如图 3 所示，总体包含五个阶段：（1）数据预处理与光谱标准化；（2）基线链条构建，即 Raw K-means、PCA + K-means、Canonical AE + K-means 和 SAE + K-means 的递进比较；（3）Geo-DEC 空间增强扩展，即在 AE 表示学习思想上加入地质指数、邻域统计和 DEC 联合聚类目标；（4）多指标评价与地质解释，包括传统聚类分离指标、空间连续性指标、光谱响应分析和自动化制图；（5）外部矿物图辅助验证，即将 1:25 万高光谱遥感矿物分布图配准至 Sentinel-2 20 m 网格，用于检查聚类图斑与矿物异常线索之间的空间一致性。
...
在缺乏野外样点和可直接数字化的大比例尺岩性地质图的条件下，本文进一步引入地质云 1:25 万高光谱遥感矿物分布图作为独立外部数据源，开展定性空间一致性辅助验证。具体流程为：下载 J46 C 002003、J46 C 002004、J46 C 003003 和 J46 C 003004 四幅相邻图件，首先根据标准图幅范围判断其与土屋-延东 AOI 的空间关系；随后对 JPG 版式图件逐幅裁取主图内框，剔除白色页边和部分图框线，并依据图幅经纬度范围进行地理配准；最后统一重投影至 Sentinel-2 影像所在的 UTM 46N（EPSG:32646）20 m 网格并裁剪至研究区范围。结果表明，当前 AOI 主要由 J46 C 002003 和 J46 C 002004 两幅北侧图件覆盖，J46 C 003003 和 J46 C 003004 因位于 AOI 南侧而无有效贡献。配准后的矿物图在 AOI 内有效覆盖率为 95.36%，剩余无覆盖区主要位于两幅图件接边处，本文保留该接边空带而不进行人为补值。最后将 SAE $z=5$, $k=6$ 聚类结果以半透明方式叠加至矿物图之上，生成图 17。

#figurewide(image("../results/geology_validation/mineral_maps/mineral_map_validation_panel.png", width: 92%), [地质云高光谱矿物图与 SAE 聚类结果的定性空间一致性验证。\
(a) 1:25 万高光谱遥感矿物分布图经主图内框裁剪、配准、拼接并裁剪至土屋-延东 AOI 后的矿物异常图；当前 AOI 主要由 J46 C 002003 和 J46 C 002004 覆盖，图幅接边处保留少量无覆盖带。(b) SAE $z=5$, $k=6$ 聚类结果叠加于矿物图之上。])

需要明确的是，图 17 的意图在于辅助判断无监督聚类图斑与白云母、绿泥石、碳酸盐、铁氧化物等矿物异常线索之间的空间关系，而非将矿物图作为传统岩性地质图或监督分类中的真值标签使用。两类数据表达对象的本质差异决定了聚类结果与高光谱矿物图之间不应被期望呈现一一对应关系。本文 SAE $z=5$, $k=6$ 聚类基于 Sentinel-2 的 10 个宽波段光谱特征，其分类结果反映的是像元在可见光--近红外--短波红外波段上的综合光谱响应、SWIR 波段间差异以及空间上下文约束下的地表单元划分（spectral-spatial surface units）。与之相对，高光谱矿物分布图基于更窄波段（通常为数十至数百纳米级）的矿物诊断性吸收特征进行矿物识别，产物为白云母、绿泥石、碳酸盐、铁氧化物等特定蚀变矿物组合的空间分布（mineral assemblages）。二者在数据源光谱分辨率、分类目标与分析粒度上均存在根本性差异，因此 cluster 与 mineral 类别之间不存在直接对应关系——一个 cluster 更可能代表具有相似综合光谱响应的地表单元，而非某一单一矿物类别。这一现象属于方法论层面的预期结果，不应被误读为聚类误差或分类精度不足。

基于上述认识，本文对该叠加分析的解释框架如下：若某一聚类类别在空间上与高光谱矿物图中的羟基矿物或铁氧化物异常带相邻或局部重合，则表明该 cluster 所对应的地表单元与蚀变矿物异常分布具有空间关联，可为该 cluster 的地质意义赋予提供独立的空间证据；反之，若某一类别主要落在矿物图中大面积背景或无数据区域，则不应对其进行过度地质解释。需要指出的是，当前所使用的是 JPG 版式图件，配准结果中仍可能保留经纬网、接边线和局部版式边缘等非数据元素；两幅有效图件在 94.5°E 附近接边处存在少量无覆盖带，本文在成果图中予以保留，以避免将插值或邻近填补误认为真实矿物信息。因此，本节将图 17 定位为定性空间一致性验证材料，而非定量精度评价依据。

综上，本节辅助验证的目的不在于评价聚类结果的分类精度，而在于检验 SAE 无监督聚类所揭示的地表单元分异模式是否与已有高光谱矿物异常分布在空间上具有合理的一致性，从而为后续地质解释提供独立辅助证据。后续若能够获取地质云原始矿物分类栅格或矢量数据，则可进一步构建 cluster-mineral 列联表，统计各 cluster 内主要矿物类别面积占比，开展更为严格的定量交叉分析。
...
#table3(
  ...
  [验证方式], [聚类指标 + 地质样点/知识], [聚类指标 + 假彩色图对照 + 高光谱矿物图辅助验证],
...
#table3(
  ...
  [地质解译以定性描述为主], [5 个波段比值定量分析 + 全自动制图 + 外部矿物图辅助叠加], [修正 B11/B12 判读方向，将 C1 降级为待验证候选线索，并通过矿物异常图进行空间一致性辅助检查],
)
]
#set text(size: 10pt)
...
#table3(
  ...
  [C1], [18.0%], [B11/B12=0.992（最低），B12/B8A=1.282（最高）；不能证明 B12 吸收], [北部带状-块状，与部分假彩色褐红色区对应，并可与矿物图局部对照], [蚀变或岩性边界候选线索], [中低（矿物图辅助支持，仍需地质图/野外验证）],
...
（1）*Cluster 1 的蚀变解释应降级为待验证候选线索*。该类 B11/B12=0.992 为 6 类中最低，但这表示 B11 低于 B12，不能证明 B12（2190 nm）处存在吸收。其支持蚀变解释的证据主要来自三点：① 该类与地质假彩色图（图 2）中部分褐红色区域存在空间对应关系，但褐红色调主要反映 B12 通道亮度/对比度，并不等同于 SWIR 吸收；② 土屋-延东斑岩铜矿的围岩蚀变以绢英岩化和青磐岩化为主，在斑岩体与围岩接触带呈带状或面状分布#super[5]，与该类北部带状-块状的空间格局有一定相似性；③ 外部高光谱矿物图（图 17）可为局部羟基矿物或铁氧化物异常提供辅助参照，但由于图件来源为 JPG 版式图且并非传统岩性地质图，该证据只能用于空间一致性检查。综合来看，C1 可作为蚀变或岩性边界候选线索，但置信度不宜表述为高，必须依赖已发表地质图或野外查证进一步标定。
...
需要强调的是，上述地质解译是基于无监督聚类结果的光谱推断。高光谱矿物图能够提供外部矿物异常线索，但它仍属于遥感解译产品，不能替代岩性地质图和野外样点。因此，最终的地质归属仍需要结合野外查证或已发表的大比例尺地质图进行标定和验证。

== 局限性与展望

本研究存在以下局限：（1）无监督聚类缺乏地面真值验证，聚类类别的地质含义需要后续标定；（2）本文新增的高光谱矿物图辅助验证仍基于 JPG 版式图件，虽然已逐幅裁取主图内框并剔除部分页边和图框线，但配准结果仍会保留经纬网、接边线和少量无覆盖区域，且图件表达对象为遥感矿物异常而非传统岩性单元，因此只能用于目视空间一致性检查，不能作为严格精度评价真值；（3）Geo-DEC 已引入 3×3 统计型空间上下文，但仍不是端到端的卷积空间建模，未来可进一步引入卷积自编码器（CAE）或图神经网络联合利用更大尺度的光谱-空间信息；（4）Geo-DEC 当前版本的 DEC 损失权重、3×3 空间窗口和增强特征组合尚未进行系统消融，因此本文只能说明“当前配置下空间连续性改善而传统分离指标下降”，尚不能精确分解地质指数、空间统计和 DEC 损失各自的贡献；（5）K-means 与 DEC 均偏向紧凑簇结构，对于光谱空间中可能存在的细长或弧形分布的簇适应性仍有限，GMM 或谱聚类可能是更合适的选择；（6）仅使用了单一时相的夏季影像，不同季节的太阳高度角和地表含水量差异可能影响光谱特征；（7）仅测试了 $z ≤ 5$ 的 bottleneck 设置，更大维度的探索尚不充分。

未来工作可在以下方向展开：（1）引入 DEM 和地形因子（坡度、坡向、阴影校正）以消除地形效应；（2）使用 ASTER 热红外波段补充岩性敏感的发射率信息；（3）获取地质云原始矿物分类栅格、矢量化矿物图层或大比例尺传统地质图，将聚类结果与外部图件进行定量一致性评估（如交叉表、调整兰德指数 ARI、归一化互信息 NMI）；（4）围绕 Geo-DEC 开展组件消融，分别测试“仅地质指数”“仅空间上下文”“AE + K-means on 28D features”“不同 DEC 权重”和“不同邻域窗口”的影响；（5）尝试对比更多降维方法（t-SNE、UMAP、变分自编码器 VAE）对聚类效果的影响；（6）在更大范围的东天山地区验证该框架的泛化能力。
...
（7）外部高光谱矿物图可作为无监督聚类解释的辅助验证资料。将地质云 1:25 万高光谱遥感矿物分布图配准至 Sentinel-2 20 m 网格后，可以检查聚类图斑与白云母、绿泥石、碳酸盐及铁氧化物等矿物异常线索之间的空间关系。当前 AOI 主要由 J46 C 002003 与 J46 C 002004 两幅北侧图件覆盖，拼接后有效覆盖率为 95.36%，图幅接边处保留少量无覆盖带。但由于该图件并非传统岩性地质图，且当前使用的是 JPG 版式图，本文仅将其作为目视辅助验证依据，而不作为严格精度评价真值。

综合以上发现，本文凝练出四条核心认识：第一，SAE+K-means 无监督聚类框架在极端干旱斑岩铜矿区依然有效，具备跨气候带和跨大地构造单元的迁移能力；第二，在该数据场景下——Sentinel-2 10 波段以线性共线性为主导、地表类别数有限（4--6 类）——浅层自编码器的聚类性能与深层 SAE 旗鼓相当，提示自编码器深度的增益具有场景依赖性；第三，B11/B12 等波段比值和外部高光谱矿物图可为蚀变或岩性异常提供候选线索，但不能脱离比值方向、空间分布、图件性质和地质图/野外验证而单独作为高置信指示标志；第四，Geo-DEC 表明在原有两阶段框架之外，引入地质先验、空间上下文和聚类目标联合优化是提升无监督地质制图空间连续性和可解释性的有效扩展方向，但并不替代 AE/SAE 作为传统聚类分离指标上的强基线。上述认识分别对应了原论文（Naagar et al., 2024）在跨区域验证、深度增益分析、定量光谱解译和空间上下文利用方面的不足。
